...
NO-DILATION VERSION: Handles *_nodilation_mask.nii.gz files.
"""
...
def find_case_masks(masks_dir, case_id):
    """Find all mask files for a given case ID (nodilation pattern)."""
    masks_dir = Path(masks_dir)
    mask_files = {}
    
    for label_id, label_name in LABEL_NAMES.items():
        # Look for nodilation pattern
        mask_pattern = f"{case_id}_{label_name}_nodilation_mask.nii.gz"
        ...
        if mask_path.exists():
            ...
            alt_pattern = f"{case_id}*{label_name}*nodilation*mask*.nii.gz"
...
def get_reference_image(case_id, reference_dir=None, masks_dir=None):
    ...
    if reference is None and masks_dir:
        masks_dir = Path(masks_dir)
        mask_files = list(masks_dir.glob(f"{case_id}_*_nodilation_mask.nii.gz"))
        if mask_files:
...
def save_combined_segmentation(combined_seg, output_path, reference_img_path=None, reference_shape=None):
    ...
    case_id = output_path.stem.replace('_medsamrefined_nodilation_seg', '').replace('_seg_nodilation', '').replace('_nodilation', '').replace('_seg', '').replace('.nii', '')
    mask_files = list(masks_dir.glob(f"{case_id}_*_nodilation_mask.nii.gz"))
...
def process_case(case_id, masks_dir, output_dir, reference_dir=None):
    ...
    # Save combined segmentation with nodilation identifier
    output_path = Path(output_dir) / f"{case_id}_seg_nodilation.nii.gz"
...
def main():
    parser = argparse.ArgumentParser(
        description="Stitch together individual MedSAM2 segmentation masks (NO-DILATION version)"
    )
    parser.add_argument(
        '--masks_dir',
        type=str,
        default='medsam2_results_nodilation',
        help='Directory containing individual mask files (default: medsam2_results_nodilation)'
    )
    parser.add_argument(
        '--output_dir',
        type=str,
        default='medsam2_results_nodilation',
        help='Directory to save combined segmentations (default: medsam2_results_nodilation)'
    )
    parser.add_argument(
        '--reference_dir',
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='Process only this specific case ID (e.g., ct_1023). Shorthand for --case_ids with one case.',
    )
    
    args = parser.parse_args()
    
    # If --case_id is provided, use it as --case_ids
    if args.case_id:
        args.case_ids = [args.case_id]
    ...
    if args.case_ids:
        ...
        if not case_ids:
            # Look for nodilation pattern
            mask_files = list(masks_dir.glob("*_*_nodilation_mask.nii.gz"))
            for mask_file in mask_files:
                # Extract case_id from filename like "ct_1023_LV_nodilation_mask.nii.gz"
                # Remove .nii.gz extension and _nodilation_mask suffix
                name_base = mask_file.name.replace('.nii.gz', '').replace('_nodilation_mask', '').replace('_mask', '')
